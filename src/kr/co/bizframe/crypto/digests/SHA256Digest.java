...
public class SHA256Digest extends GeneralDigest {
	...
	protected void processBlock() {
		...
		/*
		 * int t = 0; for(int i = 0; i < 8; i ++) { // t = 8 * i h += Sum1(e) +
		 * Ch(e, f, g) + K[t] + W[t]; d += h; h += Sum0(a) + Maj(a, b, c); ++t;
		 * 
		 * // t = 8 * i + 1 g += Sum1(d) + Ch(d, e, f) + K[t] + W[t]; c += g; g
		 * += Sum0(h) + Maj(h, a, b); ++t;
		 * 
		 * // t = 8 * i + 2 f += Sum1(c) + Ch(c, d, e) + K[t] + W[t]; b += f; f
		 * += Sum0(g) + Maj(g, h, a); ++t;
		 * 
		 * // t = 8 * i + 3 e += Sum1(b) + Ch(b, c, d) + K[t] + W[t]; a += e; e
		 * += Sum0(f) + Maj(f, g, h); ++t;
		 * 
		 * // t = 8 * i + 4 d += Sum1(a) + Ch(a, b, c) + K[t] + W[t]; h += d; d
		 * += Sum0(e) + Maj(e, f, g); ++t;
		 * 
		 * // t = 8 * i + 5 c += Sum1(h) + Ch(h, a, b) + K[t] + W[t]; g += c; c
		 * += Sum0(d) + Maj(d, e, f); ++t;
		 * 
		 * // t = 8 * i + 6 b += Sum1(g) + Ch(g, h, a) + K[t] + W[t]; f += b; b
		 * += Sum0(c) + Maj(c, d, e); ++t;
		 * 
		 * // t = 8 * i + 7 a += Sum1(f) + Ch(f, g, h) + K[t] + W[t]; e += a; a
		 * += Sum0(b) + Maj(b, c, d); ++t; }
		 */
